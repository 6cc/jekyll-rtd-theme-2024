...
function pierceElement() {
  ...
  hoverDiv.style.backgroundColor = 'rgba(249,249,249,.9)';
  hoverDiv.style.opacity = .8;
  hoverDiv.style.position = 'fixed';
  hoverDiv.style.border = '1px solid #ccc';
  hoverDiv.style.boxShadow = 'inset 0 -3em 3em rgba(0, 0, 0, 0.1),' +
    '0 0 0 2px rgb(255, 255, 255),' +
    '0.3em 0.3em 1em rgba(0, 0, 0, 0.3)';
  hoverDiv.style.zIndex = 101;
  document.body.appendChild(hoverDiv);

  document.addEventListener('mouseover', function(event) {
    const hoveredElement = event.target;
    const elementContent = hoveredElement.innerText ||
    hoveredElement.href || hoveredElement.src ||
    hoveredElement.value;

    if (elementContent) {
        hoverDiv.innerText = elementContent;
